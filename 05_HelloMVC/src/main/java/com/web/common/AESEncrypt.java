...
//양방향 암호화도 java에서 제공을 하고있다.
//java.crypto, java.security패키지에서 제공하는 클래스를 이용함.
//양방향 암호화는 key관리를 잘해야함.
//AES방식으로 대칭키를 이용 -> key가 한개
public class AESEncrypt {
	
	private static SecretKey key;//암호화, 복호화하는 key
	private String path;//key를 저장하고있는 경로
	...
	private void getGeneratedKey() {
		...
		try(ObjectOutputStream oos=new ObjectOutputStream(new FileOutputStream(this.path+"bslove.bs"))){
			...
			keygen.init(128,rnd);
			this.key=keygen.generateKey();//필드에 저장
			oos.writeObject(this.key);//파일에 저장
		}
		catch(Exception e) {
			e.printStackTrace();
		}
	...
	public static String encryptData(String oriVal) throws Exception{
		...
		cipher.init(Cipher.ENCRYPT_MODE, AESEncrypt.key);//(암호화모드, 암호화 키)
	...
	//복호화하는 메소드 선언 
	public static String decrypy(String encData) throws Exception{
